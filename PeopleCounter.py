                ...
                for i in persons:
                    if abs(x-i.getX()) <= w and abs(y-i.getY()) <= h:
                        ...
                        if i.going_UP(line_down,line_up) == True:
                            cnt_up += 1;
                            print("ID:", i.getId(), 'yukarı geçiş yaptı, zamanı:', time.strftime("%c"))
                            log.write("ID: " + str(i.getId()) + ' yukarı geçiş yaptı, zamanı: ' + time.strftime("%c") + '\n')

                        elif i.going_DOWN(line_down,line_up) == True:
                            cnt_down += 1;
                            print("ID:", i.getId(), 'aşağı geçiş yaptı, zamanı:', time.strftime("%c"))
                            log.write("ID: " + str(i.getId()) + ' aşağı geçiş yaptı, zamanı: ' + time.strftime("%c") + '\n')
